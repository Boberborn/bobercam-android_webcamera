# BobrCam Virtual Camera

BobrCam uses the Windows 11 Media Foundation virtual-camera API. The installed
camera has system lifetime, so it remains available across application exits,
sign-ins, and reboots until its uninstall command calls `IMFVirtualCamera.Remove`.

Build and install:

```powershell
dotnet build .\VCamNetSampleSource\VCamNetSampleSource.csproj -c Release -p:Platform=x64
dotnet build .\VCamNetSample\VCamNetSample.csproj -c Release -p:Platform=x64
powershell.exe -ExecutionPolicy Bypass -File .\Install-BobrCamVirtualCamera.ps1
```

Remove it from **Settings > Apps > Installed apps > BobrCam Virtual Camera**,
or run:

```powershell
powershell.exe -ExecutionPolicy Bypass -File .\Uninstall-BobrCamVirtualCamera.ps1
```

The camera is intentionally identified by Windows as a virtual camera. It does
not impersonate physical hardware or bypass services that disallow virtual
cameras.
